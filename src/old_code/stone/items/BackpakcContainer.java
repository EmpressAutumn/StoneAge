...
public class BackpakcContainer extends Container {
    ...
    BackpakcContainer(int id, @Nonnull PlayerInventory inventory, @Nonnull ItemStack current) {
        ...
        for (int y = 0; y < BackpackItem.INVENTORY_HEIGHT; y++) {
            for (int x = 0; x < BackpackItem.INVENTORY_WIDTH; x++) {
                addSlot(new SlotItemHandler(handler, x + y * BackpackItem.INVENTORY_WIDTH, 44 + x * 18, 17 + y * 18));
            }
        }
    ...
    public ItemStack transferStackInSlot(@Nonnull PlayerEntity playerIn, int index) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            if (index < BackpackItem.ITEMS) {
                if (!mergeItemStack(stack, BackpackItem.ITEMS + 1, BackpackItem.ITEMS + 36, true)) {
                    return ItemStack.EMPTY;
                }

                slot.onSlotChange(stack, itemstack);
            } else {
                if (!mergeItemStack(stack, 0, BackpackItem.ITEMS, false)) {
                    return ItemStack.EMPTY;
                }
            }
