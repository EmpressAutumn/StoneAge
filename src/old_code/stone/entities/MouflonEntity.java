...
public class MouflonEntity extends WildAnimalEntity implements TopEntityInfoProvider {
    ...
    public AgeableEntity createChild(@Nonnull AgeableEntity ageable) {
        if (Math.min(dataManager.get(WildAnimalEntity.GENERATION), ageable.getDataManager().get(WildAnimalEntity.GENERATION)) >= Config.domesticateAfterGenerations) {
            ...
            if (child != null) {
                ...
                if (result instanceof AgeableEntity) {
                    ...
                    LivingEntity.LOGGER.warn("'{}' is not instance of Ageable entity! Spawning default SHEEP entity", Config.mouflonBreedingResult);
                }
            } else {
                LivingEntity.LOGGER.warn("'{}' does not exists! Spawning default SHEEP entity", Config.mouflonBreedingResult);
            }
            ...
            if (entity != null) {
                entity.setGeneration(dataManager.get(WildAnimalEntity.GENERATION) + 1);
            }
    ...
    public void addProbeInfo(@Nonnull ProbeMode mode, @Nonnull IProbeInfo probeInfo, @Nonnull PlayerEntity player, @Nonnull World world, @Nonnull Entity entity, @Nonnull IProbeHitEntityData data) {
        probeInfo.horizontal().text("Generation: " + dataManager.get(WildAnimalEntity.GENERATION));
    }
}
