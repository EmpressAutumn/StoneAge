...
public class StoneChestContainer extends Container {
    ...
    StoneChestContainer(int id, @Nonnull BlockPos pos, @Nonnull World world, @Nonnull PlayerInventory inventory, @Nonnull PlayerEntity player) {
        ...
        tile.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY).ifPresent(h -> {
            for (int y = 0; y < StoneChestTileEntity.INVENTORY_HEIGHT; y++) {
                for (int x = 0; x < StoneChestTileEntity.INVENTORY_WIDTH; x++) {
                    addSlot(new SlotItemHandler(h, x + y * StoneChestTileEntity.INVENTORY_WIDTH, 44 + x * 18, 17 + y * 18));
                }
            }
        });
    ...
    public ItemStack transferStackInSlot(@Nonnull PlayerEntity playerIn, int index) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            if (index < StoneChestTileEntity.INVENTORY_WIDTH * StoneChestTileEntity.INVENTORY_HEIGHT) {
                if (!mergeItemStack(stack, StoneChestTileEntity.INVENTORY_WIDTH * StoneChestTileEntity.INVENTORY_HEIGHT + 1, StoneChestTileEntity.INVENTORY_WIDTH * StoneChestTileEntity.INVENTORY_HEIGHT + 36, true)) {
                    return ItemStack.EMPTY;
                }
                slot.onSlotChange(stack, itemstack);
            } else {
                if (!mergeItemStack(stack, 0, StoneChestTileEntity.INVENTORY_WIDTH * StoneChestTileEntity.INVENTORY_HEIGHT, false)) {
                    return ItemStack.EMPTY;
                }
            }
