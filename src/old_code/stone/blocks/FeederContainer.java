...
public class FeederContainer extends Container {
    ...
    FeederContainer(int id, @Nonnull BlockPos pos, @Nonnull World world, @Nonnull PlayerInventory inventory, @Nonnull PlayerEntity player) {
        ...
        tile.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY).ifPresent(h -> {
            for (int x = 0; x < FeederTileEntity.ITEMS; x++) {
    ...
    public ItemStack transferStackInSlot(@Nonnull PlayerEntity playerIn, int index) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            if (index < FeederTileEntity.ITEMS) {
                if (!mergeItemStack(stack, FeederTileEntity.ITEMS + 1, FeederTileEntity.ITEMS + 36, true)) {
                    return ItemStack.EMPTY;
                }

                slot.onSlotChange(stack, itemstack);
            } else {
                if (tile.isItemValid(stack) && !mergeItemStack(stack, 0, FeederTileEntity.ITEMS, false)) {
                    return ItemStack.EMPTY;
                }
            }
